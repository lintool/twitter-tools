...
public class QueryEnvironment {
	...
	public void runQuery(String queryName, Query query, Filter filter, int count)  {
		try {
			...
			for (ScoreDoc scoreDoc : rs.scoreDocs) {
				...
				System.out.println(queryName + " Q0 " + hit.getField("id").numericValue() + " " +
						(j++) + " " + scoreDoc.score + " lucy");
	...
	public void parseParams() throws Exception {
		...
		// make sure we've got a file specifying the index to search and a file with queries to run
